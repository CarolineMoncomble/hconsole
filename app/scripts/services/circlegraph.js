...
angular.module('hconsoleApp').factory('CircleGraph', function ($rootScope, $window) {
    ...
    var CircleGraph = function (nodeId, dataTree) {
        console.log('init de la classe CircleGraph');
        console.log('test circle : ' + dataTree.logical);
        console.log('test circle2 : ' + dataTree.physical);
        var diameter = 500,
            format = d3.format(',d'),
            margin = 4,
            padding = 0.1,
        ...
        return graph = {
            ...
            draw : function () {
                ...
                var newNode = node.enter().append('g')
                    .attr('class', function(d) { return 'node' + (d.children ?  '' : ' leaf') + (d.className ? ' ' + d.className : ''); })
                    .attr('transform', graph.getCoord)
                    .filter(function(d) { return typeof d.parent !== 'undefined'; });    //ICI !== au lieu de !=
